...
class AdminEventCalendarController extends AdminBaseController
{
    ...
    public function index()
    {
        ...
        if ((session()->get('access_token') !='') && session()->get('access_token')) {
            ...
            if ($this->client->isAccessTokenExpired()) { 
                ...
                $accessTokenUpdated =  $this->client->fetchAccessTokenWithRefreshToken(Auth::user()->cal_ref_token);
                $accessTokenUpdated =  $this->client->setAccessToken($this->client->getAccessToken()); 
                User::where('id', Auth::user()->id)->update(['access_token' =>$this->client->getAccessToken()]);
       ...
       // $service = new Google_Service_Calendar_Events($this->client);
        ...
        $synctocken=$userinfo->sync_token;
        
        if($synctocken !=""){
            $results = $service->events->listEvents($calendarId,['syncToken' => $synctocken]);

        }else{
            $results = $service->events->listEvents($calendarId);

        }
         $newtoken=$results->getNextSyncToken();

         User::where('id', Auth::user()->id)->update(['sync_token' =>$newtoken]);
          
        $res= $results->getItems();
      
        foreach($res as $allevent){
            $eventid=$allevent['id'];
            $title= $allevent['summary'];
            $description= $allevent['description'];
            $start= $allevent['start']['dateTime'];
            $end= $allevent['end']['dateTime'];
            $stratd= $allevent['end']['date'];
            $endd= $allevent['start']['date'];
            $startdate="";
            $enddate="";
            if($stratd !="" && $endd !=""){
                $startdate=  date("Y-m-d H:i:s", strtotime($stratd));
                $enddate=  date("Y-m-d H:i:s", strtotime($endd));
            }
            ...
            $eventinfo= Event::where('cal_event_id',$eventid)->get();
           if(count($eventinfo) > 0){
            ...
            $event->label_color ='bg-info';  
            $event->cal_event_id = $eventid;
            $event->save();
    ...
    public function store(StoreEvent $request)
    {
        ...
        if ((session()->get('access_token') !="") && session()->get('access_token')) {
            ...
            $event1 = new Google_Service_Calendar_Event([
                ...
                'start' => ['dateTime' => Carbon::createFromFormat($this->global->date_format, $request->start_date)->format('Y-m-d') . 'T' . Carbon::createFromFormat($this->global->time_format, $request->start_time)->format('H:i:s').'+05:30'],
                'end' => ['dateTime' => Carbon::createFromFormat($this->global->date_format, $request->end_date)->format('Y-m-d') . 'T' . Carbon::createFromFormat($this->global->time_format, $request->end_time)->format('H:i:s').'+05:30'],
        ...
        return 500;
            //return redirect('admin/logingoogle');
    ...
    public function update(UpdateEvent $request, $id)
    {
        ...
        $taskinfo=Event::where('id',$id)->first();
        $cal_event_id=$taskinfo->cal_event_id;
       
        if ((session()->get('access_token') !="") && session()->get('access_token')) {
            $this->client->setAccessToken(session()->get('access_token'));
            if ($this->client->isAccessTokenExpired()) { 
                $this->client->fetchAccessTokenWithRefreshToken(Auth::user()->cal_ref_token);
            ...
            if($cal_event_id !=""){
            $results = $service->events->update($calendarId,$cal_event_id, $event1);
            }
       ...
       $this->client->setRedirectUri($rurl);
        ...
        if (!isset($_GET['code'])) {
          ...
          // session()->put('client',  $this->client);
            ...
            User::where('id', Auth::user()->id)->update(['access_token' =>$this->client->getAccessToken(),'cal_ref_token'=>$this->client->getRefreshToken()]);
